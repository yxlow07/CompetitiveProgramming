// Author: Yu Xuan
// Created On: 12 10 2024 - 21:32:11
// File: B
...
void solve() {
    int n; cin>>n; double x[n], y[n];
    ff(i, 0, n-1) cin>>x[i]>>y[i];
    double cost = sqrt(pow(0-x[0], 2)+pow(0-y[0], 2));
    cost += sqrt(pow(0-x[n-1], 2)+pow(0-y[n-1], 2));
    
    ff(i, 1, n-1) {
        cost += sqrt(pow(x[i]-x[i-1], 2)+pow(y[i]-y[i-1], 2));
        debug(cost);
    }
    cout<<cost;
}
