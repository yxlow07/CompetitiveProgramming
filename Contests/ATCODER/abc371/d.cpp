// Author: Yu Xuan
// Created On: 18 09 2024 - 12:03:23
// File: d
...
//    cin>>tt;
    while (tt--) {
        int n, q; cin>>n; int x[n], p[n], s[n+1];
        ff(i, 0, n-1) cin >> x[i];
        ff(i, 0, n-1) cin >> p[i];
        s[0] = 0;
        ff(i, 0, n-1) s[i+1] = s[i] + p[i]; // Need to initialise the first coordinate first to prevent negative errors

        int l, r; cin >> q;
        ff(i, 0, q-1) {
            cin >> l >> r;
            int itl = upper_bound(x, x+n, l-1) - x;
            int itr = upper_bound(x, x+n, r) - x;
            int ans = s[itr] - s[itl];
            cout<<ans<<nl;
        }
    }
