// Author: Yu Xuan
// Created On: 18 09 2024 - 14:10:48
// File: c
...
//    cin>>tt;
    while (tt--) {
        ...
        vector<set<int>> edge_g(n), edge_h(n); int u, v;
        ff(i, 0, mg-1) {
            cin >> u >> v;
            --u; --v;
            edge_g[u].insert(v);
        ...
        do {
            ...
            ff(i, 0, n-1) {
                ff(j, i + 1, n-1) {
                    bool edgeInG = edge_g[i].count(j);
                    bool edgeInH = edge_h[perm[i]].count(perm[j]);

                    if (edgeInG != edgeInH) cost += a[perm[i]][perm[j]];
                }
            }
