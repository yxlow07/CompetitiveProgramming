// Author: Yu Xuan
// Created On: 20 09 2024 - 22:37:42
// File: A
...
signed main() {
    ...
    while (tt--) {
        int n, x, y; cin>>n>>x>>y;
        int time_to_add = n/y, time_to_blend = n/x;
        if (time_to_add*y < n) time_to_add++;
        if (time_to_blend*x < n) time_to_blend++;
        cout<<max(time_to_add, time_to_blend)<<nl;
    }
}