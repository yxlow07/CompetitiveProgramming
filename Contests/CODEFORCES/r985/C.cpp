// Author: Yu Xuan
// Created On: 09 11 2024 - 23:22:33
// File: C
// Link: https://codeforces.com/contest/2029/my
...
void solve() {
    int n; cin>>n; int a[n]; ff(i, 0, n-1) cin>>a[i];
    int prefix[n+1];
    memset(prefix, 0, n+1);
    int x = 0;
    ff(i, 1, n) {
    ...
    int min_suffix_x[n+2];
    memset(min_suffix_x, 0, sizeof min_suffix_x);
    min_suffix_x[n+1] = INT32_MAX;
    for(int i=n;i>=1;i--){
        min_suffix_x[i] = min(prefix[i], min_suffix_x[i+1]);
    }
    int max_diff = INT32_MIN;
    ff(i, 1, n) {
        max_diff = max(max_diff, prefix[i-1] - min_suffix_x[i]);
    }
    cout<<(prefix[n] + max_diff)<<nl;
}
