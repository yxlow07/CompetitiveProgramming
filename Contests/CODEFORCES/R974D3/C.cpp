// Author: Yu Xuan
// Created On: 21 09 2024 - 22:05:52
// File: C
...
signed main() {
    ...
    cin>>tt;
    while (tt--) {
        int n; cin>>n; int a[n], mx = -inf, total = 0, unhappyIndex;
        ff(i, 0, n-1) {
            cin>>a[i];
            mx = max(mx, a[i]);
            total += a[i];
        }
        unhappyIndex = total / (2 * n);
        ...
        // Edge case
        if (a[half] < unhappyIndex) {
            cout<<0<<nl;
            continue;
        }
        cout<<max(0LL, n*a[half]*2+1-total)<<nl;
    }
}