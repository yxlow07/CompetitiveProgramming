// Author: Yu Xuan
// Created On: 02 11 2024 - 22:52:56
// File: B
...
void solve() {
    int n,k, b, c; cin>>n>>k; mii brands; int sm = 0; vi prices;
    ff(i,0, k-1) {
        cin>>b>>c;
        brands[b] += c;
    }
    loop(b, brands) prices.push_back(b.second);
    sort(all(prices));
    int i = 1, x = size(prices), idx = min(n, x);
    while (i <= idx) {
        sm += prices[x-i];
        i++;
    }
