// Author: Yu Xuan
// Created On: 03 11 2024 - 12:48:19
// File: C_
// Link: 
...
void solve() {
    string s; int sm = 0;
    cin>>s;
    int sz = s.size(); vector<bool> pos(sz, false);
    ff(i, 0, sz-4) {
    ...
    while (q--) {
        int i, v; cin>>i>>v;
        i--;
        // Update the string and recalculate the number of 1100
        s[i] = char('0' + v);
        int l, r; l = max(0ll, i-3); r = min(i+3, (int)sz-1);
        ff(j, l, r-3) {
            if (s.substr(j, 4) == "1100") {
                if (!pos[j]) sm++, pos[j] = true;
            } else {
                if (pos[j]) sm--, pos[j] = false;
            }
        }
        cout<<(sm > 0 ? "Yes" : "No")<<nl;
    }
