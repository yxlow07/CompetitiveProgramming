// Author: Yu Xuan
// Created On: 21 10 2024 - 20:50:44
// File: 2003D1
// Link: https://codeforces.com/problemset/problem/2003/D1
...
void solve() {
    int n, m, l, t; cin>>n>>m; int k = 0LL;
    ff(i, 0, n-1) {
        cin>>l;
        initLimit(arr, 0, l+100);
        ff(j, 0, l-1) {
            cin>>t;
            if (t <= l+4) arr[t] = 1; // Rmb mex of arr cannot exceed len(arr) +4 for in case overflow
        }
        int found = 0, ptr = 0, mex = -inf;
        while (found < 2) {
            if (arr[ptr] == 0) mex = ptr, found++;
            ptr++;
        }
        k = max(k, mex);
    }
    int ans = k >= m ? (m + 1) * k : k * k + (m + k) * (m - k + 1) / 2;
    cout<<ans<<nl;
}
