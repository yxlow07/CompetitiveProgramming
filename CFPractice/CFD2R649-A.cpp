// Author: Yu Xuan
// Created On: 29 08 2024 - 17:40:34
// File: CFD2R649-A
...
signed main() {
    ...
    cin >> tt;
    while (tt--) {
        int n, x; cin>>n>>x; int a[n], p[n];
        ff(i, 0, n-1) {
            cin>>a[i];
        }
        p[0] = a[0];
        ff(i, 1, n-1) p[i] = a[i] + p[i-1];
        if (p[n-1] % x != 0) cout<<n<<nl;
        else {
            int first = -1, last = -1;
            ff(i, 0, n-1) {
                if (a[i] % x != 0 && first == -1) first = i;
                else if (a[i] % x != 0) last = i;
            }
            if (first == -1) cout<<-1<<nl;
            else cout<<max(n-1-first, last)<<nl;
        }
    }
}
