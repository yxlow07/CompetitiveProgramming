// Author: Yu Xuan
// Created On: 11 10 2024 - 20:15:03
// File: 1989C
...
void solve() {
    ...
    int rating_a = 0, rating_b = 0, same_pos = 0, same_neg = 0;
    ff(i, 0, n-1) {
        if (a[i] > b[i]) rating_a += a[i];
        else if (a[i] < b[i]) rating_b += b[i];
        else if (a[i] == -1) same_neg++;
        else if (a[i] == 1) same_pos++;
    }
    while (same_pos--) {
        if (rating_a < rating_b) rating_a++;
        else rating_b++;
    }
    while (same_neg--) {
        if (rating_a < rating_b) rating_b--;
        else rating_a--;
    }
    cout<<min(rating_a, rating_b)<<nl;
}
