// Author: Yu Xuan
// Created On: 15 10 2024 - 15:46:03
// File: 1324D
...
template <class T, class V> void print(const pair <T, V> p) {cerr << "{"; _print(p.first); cerr << ","; _print(p.second); cerr << "}";}
...
void solve() {
    int n; cin>>n; int a[n], b[n], ans = 0; vi diff(n);
    ff(i, 0, n-1) cin>>a[i];
    ff(i, 0, n-1) cin>>b[i];

    ff(i, 0, n-1) diff[i] = a[i]-b[i];
    sort(all(diff));
    debug(diff);

    ff(i, 0, n-1) {
        if (diff[i] <= 0) continue;
        int pos = lower_bound(all(diff), 1-diff[i]) - diff.begin();
        ans += ( i - pos );
    }
    cout<<ans;
}
