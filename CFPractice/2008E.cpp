// Author: Yu Xuan
// Created On: 02 10 2024 - 19:40:45
// File: 2008E
...
void solve() {
    ...
    auto calc = [&] (const string & str, const int & len) -> int {
        mci odd, even;
        ff(i, 0, len-1) {
            if (i%2 != 0) even[str[i]]++;
            else odd[str[i]]++;
        ...
        loop(x, odd) max_odd = max(max_odd, x.second);
        loop(x, even) max_even = max(max_even, x.second);

        // Calculate answer
        int ans = ((len+1) / 2 - max_odd) + (len / 2 - max_even);

        return ans;
    };

    if (n == 1) cout<<1<<nl;
    else if (n%2 == 0) cout<<calc(s, n)<<nl;
    else {
        int min_op = inf;
        ff(i, 0, n-1) {
            string sn = s;
            sn.erase(sn.begin()+i);
            int ans = calc(sn, n-1) + 1;
            min_op = min(min_op, ans);
        }
        cout<<min_op<<nl;
    }
}
