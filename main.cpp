...
int remove_first_n_digits(int num, int n) {
    string numStr = to_string(num);
    if (numStr.length() <= n) return 0; else return stol(numStr.substr(n), nullptr, 10);
}
...
bool intersect(Cuboid &x, Cuboid &y) {
    int vol;
    if (overlap(x.x1, x.x2, y.x1, y.x2) && overlap(x.y1, x.y2, y.y1, y.y2) && overlap(x.z1, x.z2, y.z1, y.z2))
        vol = calcVolume(x.x1, x.x2, y.x1, y.x2) * calcVolume(x.y1, x.y2, y.y2) * calcVolume(x.z1, x.z2, y.z1, y.z2);
    else return false;
    return vol > 0;
}

vector<int> calculateSieve(int n) {
    vector<int> sieve(n + 1, 0), result(n + 1, 0);
    for (int i = 2; i <= n; i++)
        if (sieve[i] == 0)
            for (int j = 1; j * i <= n; j++)
                if (sieve[i * j] == 0)
                    sieve[i * j] = i;
    for (int i = 2; i <= n; i++) sieve[i] = sieve[i / sieve[i]] + 1, result[i] = result[i - 1] + sieve[i];
    return result;
}
