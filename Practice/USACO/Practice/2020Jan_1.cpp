// Author: Yu Xuan
// Created On: 28 10 2024 - 19:28:43
// File: 2020Jan_1
...
void solve() {
    int n, k; cin>>n>>k;
    string s, curr_str; int str_len = 0, l;
    vector<string> lines;
    ff(i, 0, n-1) {
        cin>>s; l = s.length();
        if (str_len + l <= k) curr_str += (str_len == 0 ? "" : " ") + s, str_len += l;
        else lines.push_back(curr_str), curr_str = s, str_len = l;
    }
    if (!curr_str.empty()) lines.push_back(curr_str);
    loop(line, lines) {
        cout<<line<<endl;
    }
}
