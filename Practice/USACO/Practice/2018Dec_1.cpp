// Author: Yu Xuan
// Created On: 29 10 2024 - 18:54:26
// File: 2018Dec_1
...
void solve() {
    vpii buckets(3); // First capacity second amount
    ff(i, 0, 2) {
        cin>>buckets[i].first>>buckets[i].second;
    }
    int curr_ = 0, to_ = 1;
    ff(i, 0, 99) {
        if (buckets[curr_].second + buckets[to_].second <= buckets[to_].first) {
            // Can pour all
            buckets[to_].second += buckets[curr_].second;
            buckets[curr_].second = 0;
        } else {
            buckets[curr_].second -= (buckets[to_].first-buckets[to_].second);
            buckets[to_].second = buckets[to_].first;
        }
        curr_++, to_++, curr_ %= 3, to_ %= 3;
    }
    ff(i, 0, 2) {
        cout<<buckets[i].second<<(i != 2 ? nl : ' ');
    }
}
