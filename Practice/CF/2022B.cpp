// Author: Yu Xuan
// Created On: 07 11 2024 - 22:04:34
// File: 2022B
...
//bool ok(int &m, const vector<int>& cars, int &x) { // x = max number of cars one can buy
//    int customers[m], j = 0;
...
void solve() {
    int n, x, c, mx = -1, sum = 0; cin>>n>>x;
    vector<int> cars(n, 0);
    ff(i, 0, n-1) cin>>c, cars[i] = c, mx = max(mx, c), sum += c;
//    int l = 0, r = 1e5, res = -1;
//    // Binary search
//    while (l < r) {
//        int m = (l+r)/2; // m = number of customers
//        if (ok(m, cars, x)) res = m, l = m+1;
//        else r = m - 1;
//    }
//    assert(res != -1);
//    cout<<res<<nl;
