// Author: Yu Xuan
// Created On: 22 10 2024 - 17:59:16
// File: 1932C
// Link: 
...
void printM(int &s, int &m) {
    cout<<s%m<<" ";
}

void solve() {
    int n, m; cin>>n>>m; int a[n], totalProd;
    ff(i, 0, n-1) cin>>a[i];
    string s; cin>>s; int l = 0, r=n-1;
    ...
    assert(l == r);
    totalProd = a[l] % m; vi ans(n, 0);
    ans[n-1] = totalProd;
    fb(i, n-2, 0) {
        if (s[i] == 'L') ans[i] = (ans[i+1]*a[--l]) % m;
        else ans[i] = (a[++r]* ans[i+1]) % m;
    }
