// Author: Yu Xuan
// Created On: 13 10 2024 - 18:38:12
// File: 489C
...
bool possible(int m, int s) {
    // remaining digits can be constructed
    return s >= 0 && m * 9 >= s;
}

void solve() {
    int m, sum; cin>>m>>sum; int sum1 = sum;// m length s sum
    // Not possible case, total sum is more than allowed digit all nine
    if (m*9 < sum || sum == 0) {
        if (sum==0 && m==1) cout<<"0 0";
        else cout<<"-1 -1";
    }
    else {
        string mn, mx;
        ff(i, 0, m-1) {
            ff(j, 0, 9) {
                if (possible(m-i-1, sum-j) && !(i == 0 && j == 0)) {
                    sum -= j, mn += char('0'+j);
                    break;
                }
            }
