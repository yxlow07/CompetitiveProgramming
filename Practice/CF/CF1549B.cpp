// Author: Yu Xuan
// Created On: 20 09 2024 - 18:59:23
// File: CF1549B
...
signed main() {
    ...
    while (tt--) {
        int n; cin >> n;
        string en, sf; cin>>en>>sf;
        bool reached[n], enemy[n], self[n];
        int count = 0;
        ff(i, 0, n-1) {
            reached[i] = enemy[i] = self[i] = false;
        }
        ff(i, 0, n-1) {
            enemy[i] = (en[i] == '1');
            self[i] = (sf[i] == '1');
        ...
        ff(i, 0, n-1) {
            if (self[i]) {
                if (i-1 >= 0 && !reached[i-1] && enemy[i-1]) {
                    count++; reached[i-1] = true;
                } else if (!reached[i] && !enemy[i]) {
                    count++; reached[i] = true;
                } else if (i+1 <= n-1 && !reached[i+1] && enemy[i+1]) {
                    count ++; reached[i+1] = true;
                }
            }
        }
