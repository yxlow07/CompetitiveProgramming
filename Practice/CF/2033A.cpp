// Author: Yu Xuan
// Created On: 15 11 2024 - 16:44:58
// File: 2033A
...
void fast(const string &file = "") {
    ...
    if (!freopen((current_path + "/inp.txt").c_str(), "r", stdin)) {
        cerr << "Error opening input file\n" << current_path << '\n';
        exit(1);
    }
    if (!freopen((current_path + "/out.txt").c_str(), "w", stdout)) {
        cerr << "Error opening output file\n" << current_path << '\n';
        exit(1);
    }
    if (!freopen((current_path + "/err.txt").c_str(), "w", stderr)) {
...
void solve() {
    int n; cin>>n; string s = "Sakurako", k = "Kosuke";
    int x=0, i = 1, p = -1;
    while (abs(x) < n) {
        x += p * (2*i-1);
        p *= -1, i++;
