// Author: Yu Xuan
// Created On: 23 12 2024 - 23:16:24
// File: 2019A
...
void solve() {
    ...
    // Put the elements odd position into vector b and even into vector c
    vector<int> b, c; int mx_b = -1, mx_c = -1;
    ff(i, 0, n-1) {
        if (i&1) c.push_back(a[i]), mx_c = max(mx_c, a[i]);
        else b.push_back(a[i]), mx_b = max(mx_b, a[i]);
    }
    cout<<max(mx_b+b.size(), mx_c+c.size())<<nl;
}
