// Author: Yu Xuan
// Created On: 10 11 2024 - 15:05:28
// File: Innovation
// Link: https://practice.ioimalaysia.org/problems/4
...
void solve() {
    ...
    vi prefix(n+1,0);
    ff(i, 0, n-1) {
        prefix[i+1] = prefix[i] + cards[i].ab;
    }
    int ans = 0;
    ff(i, 0, m-1) {
        int total = (prefix[m] - cards[i].ab) + (cards[i].c + cards[i].d);
        ans = max(ans, total);
    }
    ff(i, m, n-1){
        if(m >= 1){
            int total = prefix[m - 1] + cards[i].ab + cards[i].c + cards[i].d;
            ans = max(ans, total);
        }
    }
    cout<<ans<<nl;
