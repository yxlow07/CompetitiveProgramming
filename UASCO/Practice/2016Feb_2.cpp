// Author: Yu Xuan
// Created On: 29 10 2024 - 21:25:31
// File: 2016Feb_2
...
void solve() {
    int n; cin>>n; int rooms[n];
    ff(i, 0, n-1) cin>>rooms[i];

    // Bruteforce the solution
    int best = inf, curr = 0, dist = 1;

    ff(i, 0, n-1) {
        // Calculate the distance starting at each door
        ff(j, i+1, n-1) {
            curr += (dist * rooms[j]);
            dist++;
        }
        ...
        cerr<<"Best distance starting at "<<rooms[i]<<" has best distance of "<<curr<<nl;
        best = min(best, curr);
        curr = 0, dist = 1;
    }
